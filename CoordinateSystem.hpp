...
namespace quakins {
	...
	struct CoordinateSystemHost {
		
		CoordinateSystemHost(std::array<std::size_t,dim> nz,
	...
	template <typename val_type, std::size_t dim>
	struct CoordinateSystemDevice {
		
		CoordinateSystemDevice(CoordinateSystemHost
										<val_type,dim>& coordHost) {
			thrust::copy(coordHost.coord.begin(),
											coordHost.coord.end(),coord.begin());
		}	
	
		std::array<thrust::device_vector<val_type>,dim> coord;
